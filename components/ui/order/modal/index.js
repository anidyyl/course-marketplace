...
const createFormState = ({price, email, confirmationEmail}, hasAgreedTOS, isNewPurchase) => {
  ...
  } else if ((confirmationEmail.length === 0 || email.length === 0) && isNewPurchase) {
    return _createFormState(true)
  } else if (email !== confirmationEmail && isNewPurchase) {
    return _createFormState(true, "Email are not matching")
  } else if (!hasAgreedTOS) {
    return _createFormState(true, "You need to agree with terms of services in order to submit the form")
  }
